...
- [ ] Пересоздать SECRET_KEY в ska/settings.py и переместить его в отдельное хранилище вне репозитория
- [ ] Определить ALLOWED_HOSTS в ska/settings.py и переместить его в отдельное хранилище вне репозитория
- [ ] Подключить систему хранения секретных параметров (на выбор):
  - Docker Secrets
  - HashiCorp Vault
  - Ansible Vault
- [ ] Подключить Docker Swarm